...
CREATE TABLE `payouts` (
  ...
  `amount` decimal(12,2) NOT NULL,
  `currency` char(3) NOT NULL DEFAULT 'TZS',
  `status` enum('pending','approved','submitted','paid','failed','reversed') NOT NULL DEFAULT 'pending',
  `provider_reference` varchar(120) DEFAULT NULL,
  `admin_note` varchar(1000) DEFAULT NULL,
  `created_at` datetime NOT NULL DEFAULT current_timestamp(),
...
CREATE TABLE `tournaments` (
  ...
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_tournaments_share_token` (`share_token`),
...
-- Dump completed on 2026-09-19 10:11:32
